...
#'
#'
estimateGremlinsModel <- function(data, design, R, Priors = NULL, nSegments = 2, nBetaSegments = 2, covariates = NULL, constraints = NULL, segmentCovariates = NULL, startingValues = NULL) {
  ...
  gremlinsEnv$acceptanceRateLambda <- rep(0, times=gremlinsEnv$nSegments) # One per lambda (first one does not count)
  ...
  for(rep in 2:R) {
    ...
    for(ind in 1:gremlinsEnv$nUnits) {
      ...
      # K[rep, ind] <- startingValues$segMembership[ind]
    ...
    lambda[rep,] <- generateLambdaMix(lambda[rep - 1,], data, design, slope[rep,,], K[rep-1,], Priors)
...
generateIndividualSlope <- function(data, design, currentSlope, lambda, slopeBar, slopeCov, constraints) {
  ...
  proposedLL <- llmnl(currentSlope/lambda, data, design)
...
generateLambdaMix <- function(currentLambda, data, design, currentSlope, K, priors) {
  ...
  for(k in 2:gremlinsEnv$nSegments) {
    ...
    for(ind in 1:gremlinsEnv$nUnits) {
      if(K[ind] == k) {
        ...
        cIndLL[ind] <- llmnl(currentSlope[ind,]/lambda[k], data[ind, -c(1:2)], design[design[,1] == data[ind, 2], -c(1:3)])
        pIndLL[ind] <- llmnl(currentSlope[ind,]/proposedLambda, data[ind, -c(1:2)], design[design[,1] == data[ind, 2], -c(1:3)])
      }
    }
    ...
    if(log(runif(1)) < lap) {
      ...
      gremlinsEnv$acceptanceRateLambda[k] <- gremlinsEnv$acceptanceRateLambda[k] + 1
...
gremlinsLogLikelihood <- function(data, design, slope, lambda) {
  ...
  n <- gremlinsEnv$nTasks
  j <- gremlinsEnv$nConcepts
  num <- utils[0:(n-1)*j + data]

  utils <- matrix(utils, nrow=j)
  denom <-  colSums(exp(utils))

  return(sum(num - log(denom)))
}
